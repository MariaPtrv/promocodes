...
1. Необходимо установить переменные среды для сервисов admin и promocodes. Шаблон .env файла находится в директории `/configs` каждого проекта.

2. Для запуска сервисов admin, promocodes и базы данных достаточно выполнить команду в корне проекта:
...
3. Для запуска UI необходимо перейти в директорию /ui 
```
cd ui
```
...
4. Для сервиса `promocodes` есть возможность запустить тесты, выполнив в директории сервиса команду:
